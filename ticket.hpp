...
public:
    ...
    int buy_ticket(char* u,char* i,int mm,int dd,int n,char* f,char* t,bool q,train &trains,user &users,int times,ticket_base &ticket_base_){
        if(users.login_set.find(username(u))==users.login_set.end())return -1;//用户没登录不能买票
        ...
        // std::cout<<"**************"<<std::endl;
        if(!result[0].release)return -1;//没放票不能买票
        ...
        if(no1==result[0].stationNum || no2==result[0].stationNum || no1>=no2)return -1;
        ...
        if(x<st || x>ed)return -1;//判断发车时间是否合法
